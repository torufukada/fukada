...
GitHub の **リポジトリのトップページ** で、緑の **Code** ボタンを開き **Download ZIP** を選んでください。
もし ZIP の中身が `docs/*.md` だけになってしまう場合は、**個別ファイルのページから落としている**可能性が高いです。

直リンクで落とす場合は、GitHub で表示されている **ブランチ名** を使って以下の形式にしてください。

```
https://github.com/<org_or_user>/<repo>/archive/refs/heads/<branch>.zip
```

## ZIP を自分で作成（チャット転送・共有用）

GitHub から落とせない場合は、リポジトリ内で ZIP を生成できます。

```bash
python scripts/build_zip.py --output fukada.zip
```

生成された `fukada.zip` を共有すれば、**中身が欠けない完全な実行ファイル一式**を渡せます。
